...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        Run newRun = new Run(this.transaction, this.computeSchema());
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Run newRun = new Run(transaction, computeSchema());
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i < rsize; i += numBuffers - 1) {
            Run run;
            if (i + numBuffers - 1 > rsize) {
                run = mergeSortedRuns(runs.subList(i, rsize - i));
            } else {
                run = mergeSortedRuns(runs.subList(i, i + numBuffers - 1));
            }
            temp.add(run);
        }
        return temp;
    ...
    public Run sort() {
        ...
        BacktrackingIterator<Record> backtrackIter = getBlockIterator(sourceIterator, computeSchema(), numBuffers);
        return sortRun(backtrackIter);
    }
