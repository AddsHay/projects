...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        int i = numLessThanEqual(key, keys);
        BPlusNode tile = getChild(i);
        Optional<Pair<DataBox, Long>> rval = Optional.empty();
        Optional<Pair<DataBox, Long>> rOp = tile.put(key, rid);
        if (rOp.isPresent()) {
            Pair<DataBox, Long> rpair = rOp.get();
            DataBox rkey = rpair.getFirst();
            long rPageNum = rpair.getSecond();
            keys.add(i, rkey);
            children.add(i + 1, rPageNum);
            int d = metadata.getOrder();
            if (keys.size() > 2 * d) {
                int s = keys.size() / 2;
                DataBox upkey = keys.get(s);
                List<DataBox> keys2 = keys.subList(s + 1, keys.size());
                List<Long> children2 = children.subList(s + 1, children.size());
                InnerNode n = new InnerNode(metadata, bufferManager, keys2, children2, treeContext);
                keys = keys.subList(0, s);
                children = children.subList(0, s + 1);
                rval = Optional.of(new Pair<DataBox, Long>(upkey, n.getPage().getPageNum()));
            }
        }
        sync();
        return rval;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int size = keys.size();
        while (data.hasNext() & (size <= order)) {
            BPlusNode rightmost = getChild(children.size() - 1);
            Optional<Pair<DataBox, Long>> output = rightmost.bulkLoad(data, fillFactor);
            if (output.isPresent()) {
                Pair<DataBox, Long> p = output.get();
                keys.add(p.getFirst());
        ...
        List<DataBox> keys2 = keys.subList(order + 1, keys.size());
        List<Long> children2 = children.subList(order + 1, children.size());
        InnerNode n = new InnerNode(metadata, bufferManager, keys2, children2, treeContext);
        keys = keys.subList(0, order);
        children = children.subList(0, order + 1);
        sync();
        return Optional.of(new Pair<DataBox, Long>(upkey, n.getPage().getPageNum()));
    }
