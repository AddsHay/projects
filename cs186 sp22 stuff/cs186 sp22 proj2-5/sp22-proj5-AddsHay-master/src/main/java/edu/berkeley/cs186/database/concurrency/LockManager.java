...
public class LockManager {
    ...
    private class ResourceEntry {
        ...
        public void grantOrUpdateLock(Lock lock) {
            List<Lock> ls = transactionLocks.getOrDefault(lock.transactionNum, new ArrayList<>());
            ls.add(lock);
            transactionLocks.put(lock.transactionNum, ls);

            if (locks.size() > 0 && transactionLocks.get(lock.transactionNum).size() > 1) {
        ...
        public void grantorupdate2(Lock lock) {
            List<Lock> ls = transactionLocks.getOrDefault(lock.transactionNum, new ArrayList<>());
            ls.add(lock);
            transactionLocks.put(lock.transactionNum, ls);
            locks.add(lock);
    ...
    public void acquireAndRelease(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < releaseNames.size(); i++) {
                if (getLockType(transaction, releaseNames.get(i)).equals(LockType.NL)) {
            ...
            if (resourceEntry.checkCompatible(lockType, transaction.getTransNum())) {
                resourceEntry.grantorupdate2(newLock);
                ...
                resourceEntry.addToQueue(lr, true);
    ...
    public void acquire2(TransactionContext transaction, ResourceName name,
    ...
    public void acquire(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            if (resourceEntry.waitingQueue.isEmpty() && resourceEntry.checkCompatible(lockType, transaction.getTransNum())) {
                ...
                shouldBlock = true;
                LockRequest lr = new LockRequest(transaction, newLock);
                resourceEntry.addToQueue(lr, false);
                transaction.prepareBlock();
            }
        }
        if (shouldBlock) {
    ...
    public void release(TransactionContext transaction, ResourceName name)
        ...
        synchronized (this) {
            if (getLockType(transaction, name) == (LockType.NL)) {
                throw new NoLockHeldException("Dup lock");
            }
            ...
            /**
             ...
             Lock newLock = new Lock(name, lockType, transaction.getTransNum());
             resourceEntry.releaseLock(newLock);
             */
        }
    }
    ...
    public void promote(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            List<Lock> relocks = re.locks;
            Lock l = null;
            for (Lock lock : relocks) {
            ...
            if (getLockType(transaction, name).equals(LockType.NL)) {
                throw new NoLockHeldException("No lock held");
            } else if (l.lockType == newLockType) {
                throw new DuplicateLockRequestException("duplicate lockType request");
            } else if (!LockType.substitutable(newLockType, l.lockType)) {
                throw new InvalidLockException("lock is not substitutable");
            }
