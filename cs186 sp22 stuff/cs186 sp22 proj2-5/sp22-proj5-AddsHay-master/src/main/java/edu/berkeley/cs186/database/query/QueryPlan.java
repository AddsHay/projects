...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i = 0; i < cols.size(); i++) {
            ...
            IndexScanOperator indop = new IndexScanOperator(transaction, table, selectPredicates.get(index).column, selectPredicates.get(index).operator, selectPredicates.get(index).value);
            if (indop.estimateIOCost() < IOcost) {
                minOp = indop;
                IOcost = indop.estimateIOCost();
                minOpI = index;
            }
        }
        minOp = addEligibleSelections(minOp, minOpI);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> set: prevMap.keySet()) {
            for (JoinPredicate joinPredicate: this.joinPredicates) {
                ...
                String leftColumnName = joinPredicate.leftColumn;
                String rightColumnName = joinPredicate.rightColumn;
                QueryOperator rightQueryOperator = prevMap.get(set);
                QueryOperator leftQueryOperator = prevMap.get(set);
                Set<String> newTable = new HashSet<>();
                newTable.addAll(set);
                if (set.contains(leftTableName) && !set.contains(rightTableName)) {
                    Set<String> rightTable = new HashSet<>();
                    rightTable.add(rightTableName);
                    rightQueryOperator = pass1Map.get(rightTable);
                }
                else if (!set.contains(leftTableName) && set.contains(rightTableName)) {
                    Set<String> leftTable = new HashSet<>();
                    leftTable.add(leftTableName);
                    leftQueryOperator = pass1Map.get(leftTable);
                } else {
                    continue;
                }
                QueryOperator newOperator = minCostJoinType(leftQueryOperator, rightQueryOperator, leftColumnName, rightColumnName);
                newTable.add(leftTableName);
                newTable.add(rightTableName);
                if (result.get(newTable) != null) {
                    if (newOperator.estimateIOCost() < result.get(newTable).estimateIOCost()) {
                        result.put(newTable, newOperator);
                    }
                }
                result.put(newTable, newOperator);
            }
    ...
    public Iterator<Record> execute() {
        ...
        for (String tableName: this.tableNames) {
            QueryOperator table = minCostSingleAccess(tableName);
            Set<String> setTable = new HashSet<>();
            setTable.add(tableName);
            pass1Map.put(setTable, table);
        }
