...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("Duplicates not allowed!");
        }
        ...
        if (keys.size() > 2 * d) {
            ...
            List<DataBox> keys2 = keys.subList(split, keys.size());
            List<RecordId> rids2 = rids.subList(split, keys.size());
            LeafNode n = new LeafNode(metadata, bufferManager, keys2, rids2, rightSibling, treeContext);
            keys = keys.subList(0, split);
            rids = rids.subList(0, split);
            rightSibling = Optional.of(n.getPage().getPageNum());
            rval = Optional.of(new Pair<DataBox, Long>(keys2.get(0), n.getPage().getPageNum()));
        }
        sync();
        return rval;
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int capacity = (int) ((int) (2 * order) * fillFactor);
        int size = keys.size();
        while (data.hasNext() & (size < capacity)) {
        ...
        if (!data.hasNext()) {
            ...
            keys2.add(pair.getFirst());
            List<RecordId> rids2 = new ArrayList<>();
            rids2.add(pair.getSecond());
            LeafNode n = new LeafNode(metadata, bufferManager, keys2, rids2, Optional.empty(), treeContext);
            rightSibling = Optional.of(n.getPage().getPageNum());
            sync();
            return Optional.of(new Pair<DataBox, Long>(keys2.get(0), n.getPage().getPageNum()));
        }
    }
