...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> nOp = root.put(key, rid);
        if (!nOp.isPresent()) {
            return;
        }
        Pair<DataBox, Long> nPair = nOp.get();
        List<DataBox> keys = new ArrayList<>();
        List<Long> children = new ArrayList<>();
        keys.add(nPair.getFirst());
        children.add(root.getPage().getPageNum());
        children.add(nPair.getSecond());
        InnerNode n = new InnerNode(metadata, bufferManager, keys, children, lockContext);
        updateRoot(n);
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        LeafNode leftmost = this.root.getLeftmostLeaf();
        if (leftmost.scanAll().hasNext() || leftmost != this.root) {
            throw new BPlusTreeException("Populated when it should not be.");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> output = this.root.bulkLoad(data, fillFactor);
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            if (output.isPresent()) {
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            this.curr = root.getLeftmostLeaf();
            this.iterator = curr.scanAll();
            if (!this.iterator.hasNext()) {
                move();
            }
        }
        public BPlusTreeIterator(DataBox key) {
            this.curr = root.get(key);
            this.iterator = curr.scanGreaterEqual(key);
            if (!this.iterator.hasNext()) {
                move();
            }
        }
        public void move() {
            Optional<LeafNode> rs = curr.getRightSibling();
            if (rs.isPresent()) {
                this.curr = rs.get();
                this.iterator = curr.scanAll();
            } else {
