...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        currentbees = currentplace.bees
        placesaway = 0
        while currentplace.entrance is not beehive:
            if len(currentbees) > 0 and self.min_range <= placesaway <= self.max_range:
                break
            currentplace = currentplace.entrance
            currentbees = currentplace.bees
            placesaway += 1
        if self.min_range <= placesaway <= self.max_range:
            return rANTdom_else_none(currentbees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        fireplace = self.place
        Ant.reduce_armor(self, amount)
        if self.armor > 0:
            damagedbees = fireplace.bees[:]
            for bees in damagedbees:
                Insect.reduce_armor(bees, amount)
        if self.armor <= 0:
            deadbees = fireplace.bees[:]
            for bees in deadbees:
                bees.reduce_armor(self.damage + amount)
        # END Problem 5
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.realq = QueenAnt.realq
        QueenAnt.realq = False
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.realq:
            self.throw_at(self.nearest_bee(gamestate.beehive))
            nextplacebehind = self.place.exit
            for ants in [nextplacebehind.ant]:
                nextplacebehind.ant.damage = 2 * nextplacebehind.ant.damage
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        self.armor -= amount
        if self.armor <= 0 and not self.realq:
            self.place.remove_insect(self)
        if self.armor <= 0 and self.realq:
            bees_win()
        # END Problem EC
