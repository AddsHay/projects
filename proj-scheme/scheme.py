...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        part1 = scheme_eval(first, env)
        part2 = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(part1, part2, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        place = Pair(target.rest, expressions.rest)
        lamb = do_lambda_form(place, env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    un = expressions.rest
    return LambdaProcedure(formals, un, env)
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.rest != nil:
            ...
            ans = scheme_eval(expressions.first, env, True)
